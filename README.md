...
- 解压目标默认为当前文件夹；目标目录输入框留空即可，也可填写完整路径改到其他目录。批次首次提交前会显示夸克预览框中的实际目标供确认。
- 可选择在页面确认单个任务解压完成后，将对应源压缩包移入回收站；该选项默认关闭。
- 可递归扫描当前目录的所有子文件夹，把常见格式的视频移动到当前目录根层。
- 根目录或其他待移动视频存在同名文件时自动跳过，避免覆盖。
- 可选择在移动完成后删除因此变空的文件夹；该选项默认关闭，非空文件夹会保留。

### 使用

1. 打开夸克网盘文件列表页面。
2. 目标目录默认是当前文件夹；如需解压到其他位置，再填写完整目录路径。
3. 点击「扫描」确认待处理压缩包数量。
4. 如需清理源文件，勾选「解压完成后删除源压缩包（移入回收站）」。
5. 点击「开始云解压」，并根据面板日志检查提交、跳过、完成和删除结果。

归集子目录视频：

1. 在要归集视频的根目录打开脚本面板。
2. 点击「扫描子目录视频」，核对可移动数量和同名跳过数量。
3. 如需清理目录，勾选「删除因移动而变空的文件夹」。
4. 点击「移动到当前目录」，在确认框中复核操作范围后执行。

### 注意

- 脚本依赖夸克网盘当前网页交互和弹窗结构；页面改版后可能需要更新选择器。
- 批量执行前请先核对目标目录。停止操作只阻止继续提交，不会撤销已提交的任务。
- 自动删除只在页面状态同时包含对应压缩包名称和“解压完成/成功”时执行；无法确认、超时或失败时保留源压缩包。
- 空文件夹只会在移动完成并重新确认目录为空后删除；原本无关的空目录不会被处理。
